...
import com.yikang.protal.dao.MessageDao;
import com.yikang.protal.entity.Message;
...
public class MessageManager {
	...
	public int insertSystemFollowMessage(Long toUserId,String title,String content,Byte contentGroup) throws Exception{
		Date currentDateTime=Calendar.getInstance().getTime();
		Message message=new Message();
		message.setCreateTime(currentDateTime);
		message.setToUserId(toUserId);
		...
		message.setUpdateTime(currentDateTime);
		message.setContentGroup(contentGroup);
		message.setMessageGroup(Byte.valueOf("0")); 
		message.setCreateTimeMillisecond(currentDateTime.getTime());
		return	messageDao.insertSelective(message);
	}
